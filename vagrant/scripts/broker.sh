...
default.replication.factor=3/' /opt/kafka_2.10-0.8.1.1/config/server.properties
sed -i -e "s/^#*zookeeper\.connect=.*/zookeeper.connect=192.168.56.2:2181/" \
    /opt/kafka_2.10-0.8.1.1/config/server.properties
sed -i -e 's/-Xmx1G/-Xmx1G/' /opt/kafka_2.10-0.8.1.1/bin/kafka-server-start.sh
sed -i -e 's/-Xms1G/-Xms1G/' /opt/kafka_2.10-0.8.1.1/bin/kafka-server-start.sh
