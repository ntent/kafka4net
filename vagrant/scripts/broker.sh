...
sed -i -e "s|^#*listeners=.*|listeners=PLAINTEXT://192.168.56.${id}0:9092|" "/opt/kafka_2.10-${kafka_version}/config/server.properties"
...
echo 'message.max.bytes=50000000' >> "/opt/kafka_2.10-${kafka_version}/config/server.properties"
echo 'replica.fetch.max.bytes=50000000' >> "/opt/kafka_2.10-${kafka_version}/config/server.properties"
